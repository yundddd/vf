...
void* realloc(void* old_ptr, size_t new_size) {
  ...
  ret = vf::malloc(new_size);
  if (__builtin_expect(!ret, 0)) {
    return nullptr;
  }

  vf::memcpy(ret, heap->user_p, heap->len);
  munmap(heap, heap->len);
  return ret;
}
...
void* operator new(size_t n) { return vf::malloc(n); }
void* operator new[](size_t n) { return vf::malloc(n); }
void operator delete(void* p) noexcept { vf::free(p); }
void operator delete(void* ptr, unsigned long) noexcept { vf::free(ptr); }
void operator delete[](void* m) noexcept { vf::free(m); }
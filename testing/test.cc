...
TempString TypeToString(void const* value) {
  if (value == nullptr) return TempString("(nullptr)");
  TempString tempString;
...
static void locCopyStringWithElipse(char dest[STRING_EQ_PRINT_LENGTH],
  ...
  for (; i < STRING_EQ_PRINT_LENGTH - 1 && start[i]; ++i) {
    if (i < 3 && start > string)
      ...
      dest[i] = '\\';  // simply replace this with '\', we're just aiming for a
                       // general idea not an exact representation
    else
      dest[i] = start[i];
  }
...
bool TestFixture::TestStrings(char const* left, char const* right,
  ...
  if (left == nullptr || right == nullptr) {
    ...
    for (; *testLeft && *testRight; ++offset, ++testLeft, ++testRight) {
      if (*testLeft != *testRight) break;
    }

    // reached the end of both strings, so they're the same
    if (!*testLeft && !*testRight) return true;
...
static bool locExecuteTest(TestFixture* test, TestFixture::OutputMode output) {
  ...
  if (test->ExecuteTest()) {
    if (output == TestFixture::Verbose) {
      ...
      auto usec = end.tv_usec - start.tv_usec;
      float diff = (float)sec + (float)usec / 1000000;
      TestFixture::Printf(": Passed %d out of %d tests in %g seconds\n",
                          test->NumTests(), test->NumTests(), diff);
    }
    return true;
  }
...
bool TestFixture::ExecuteAllTests(char const* groupFilter,
  ...
  if (output != Silent) {
    if (count == 0)
      ...
      Printf(
          "%d Tests finished, %d of %d assertions failed. Some tests are "
          "reporting errors.\n",
          count, fails, passes);
  }
  return passed;
